...
export async function POST(
  ...
  const res = await backendFetch(
    `/connections/sources/${encodeURIComponent(source_id)}/authorize?project_id=${encodeURIComponent(projectId)}`,
  ...
  if (!res.ok) {
    const { message } = parseApiErrorBody(parsed, "source_authorize_failed");

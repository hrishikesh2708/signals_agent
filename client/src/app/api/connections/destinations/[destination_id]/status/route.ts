...
export async function GET(
  ...
  const res = await backendFetch(
    `/connections/destinations/${encodeURIComponent(destination_id)}/status?project_id=${encodeURIComponent(projectId)}`,
  ...
  if (!res.ok) {
    const { message } = parseApiErrorBody(parsed, "destination_status_failed");
